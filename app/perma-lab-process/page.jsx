...
export const metadata = buildPublicPageMetadata({
  ...
  title: "PERMA Lab Process™ | Large-Capacity Split-System Lab Chillers",
  description: "Large-capacity general lab temperature control with a remote condenser. PERMA Lab Process split-system chillers minimize lab-side footprint, with mid-temperature and low-temperature configurations down to −40 °C.",
...
const planning = [
  ["01", "Your temperature", "The operating setpoint, starting temperature, and temperature range your process needs."],
  ["02", "Your process load", "The equipment being cooled, heat load, process volume, and required cooling time."],
  ["03", "Your circulation needs", "The process fluid, required flow, and pressure requirements at your equipment."],
  ["04", "Your installation", "Available power, remote condenser location, ambient conditions, and the route between units."]
];
  ...
  return (
    <main className="site-shell lab-page">
      <StructuredData data={{ "@context": "https://schema.org", "@graph": [
        { "@type": "WebPage", "@id": "https://perma.cool/perma-lab-process#webpage", url: "https://perma.cool/perma-lab-process", name: "PERMA Lab Process™ — Split-System Lab Chillers", description: "Large-capacity general lab temperature control using a split-system process layout and remote condenser. Mid-temperature and low-temperature configurations down to −40 °C, subject to system selection.", about: { "@type": "Thing", name: "Split-system laboratory process cooling" }, publisher: { "@id": "https://perma.cool/#organization" } },
      ...
      <section className="lab-hero" aria-labelledby="lab-title">
        <div className="lab-wrap lab-hero-top"><p className="lab-kicker"><span/> A DISTINCT PERMA COOL PRODUCT LINE</p><span className="lab-hero-index">LABORATORY / PROCESS COOLING</span></div>
        <div className="lab-wrap lab-hero-grid">
          <div className="lab-hero-copy">
            <p className="lab-product-name">PERMA <span>Lab Process</span><sup>™</sup></p>
            <h1 id="lab-title">Large cooling capacity.<br/><em>Small lab footprint.</em></h1>
            <p className="lab-lead">General lab temperature control, built as a split-system process chiller with a remote condenser. Keep heat rejection out of your workspace and leave more room for the work that matters.</p>
        ...
        <div className="lab-wrap lab-hero-specs"><div><FlaskConical/><span>LAB PROCESS<span>Dedicated product family</span></span></div><div><Thermometer/><span>MID → LOW TEMP<span>Down to −40 °C</span></span></div><div><Fan/><span>REMOTE CONDENSER<span>Separate heat rejection</span></span></div></div>
      </section>
      ...
      <section className="lab-wrap lab-section lab-architecture" id="split-system" aria-labelledby="lab-architecture-title">
        <div className="lab-section-heading"><p className="lab-kicker">01 / THE ARCHITECTURE</p><h2 id="lab-architecture-title">Two locations.<br/><em>One cooling system.</em></h2><p>Keep process cooling near the work. Locate the condenser separately, with the arrangement planned around your lab and facility.</p></div>
        <div className="lab-location-cards">
          <article><span className="lab-location-number">01</span><FlaskConical size={30}/><p className="lab-kicker">AT THE PROCESS</p><h3>Lab-side chiller</h3><p>The process cooling side serves your lab equipment. Temperature, circulation, and capacity are selected around the application.</p><span className="lab-card-foot"><MapPin size={15}/> Positioned for your workflow</span></article>
          <div className="lab-connection" aria-label="Connected by refrigeration lines"><span/>Refrigeration lines<span/></div>
          <article><span className="lab-location-number">02</span><Wind size={30}/><p className="lab-kicker">AT THE REMOTE LOCATION</p><h3>Remote condenser</h3><p>The condenser rejects heat at a separate location. Placement, airflow, ambient conditions, and line routing are part of the system design.</p><span className="lab-card-foot"><MapPin size={15}/> Planned around your facility</span></article>
        </div>
      </section>

      <section className="lab-wrap lab-split-visual" aria-label="Split-system process arrangement"><SplitSystemVisual/><div><p className="lab-kicker">COOL THE PROCESS. NOT THE WHOLE ROOM.</p><h2>Put the heat<br/>somewhere else.</h2><p>A remote condenser moves condenser heat rejection away from the lab. That helps keep added heat out of the workspace, while a compact lab-side arrangement leaves more room for your process equipment.</p><p className="lab-fineprint-light">Final lab footprint, condenser location, and facility cooling needs depend on the selected configuration and installation.</p></div></section>

      <section className="lab-equipment-section"><div className="lab-wrap lab-equipment-grid"><figure><Image src="/images/generated/blast60-component-02-flux-box-wall.png" alt="Existing Perma Cool wall-mounted plate heat-exchanger assembly used as a Lab Process design reference" width={1477} height={1066} sizes="(max-width: 680px) 95vw, 650px"/><figcaption>Existing plate-system reference · Final configuration to be confirmed</figcaption></figure><div><p className="lab-kicker">A FAMILIAR PLATFORM. A NEW LAB FOCUS.</p><h2>Skid and plate systems.<br/>Planned for your lab.</h2><p>PERMA Lab Process™ starts with Perma Cool’s skid and plate-system approach, applied to large-capacity general lab temperature control.</p><p>Plan the process-side equipment around the space you have. Pair it with a remote condenser arrangement suited to your facility.</p><a className="lab-text-link" href={inquiry}>Talk through your layout <ArrowUpRight size={18}/></a></div></div></section>
      ...
      <section className="lab-temperature" id="temperature" aria-labelledby="lab-temperature-title"><div className="lab-wrap lab-section">
        <div className="lab-heading-row"><div><p className="lab-kicker">02 / TEMPERATURE DUTY</p><h2 id="lab-temperature-title">Start with the process.<br/>Define the cold.</h2></div><p>Mid-temperature and low-temperature configurations. The right selection starts with your required operating point—not just the lowest number on a label.</p></div>
        <div className="lab-temperature-grid">
          <article><Thermometer size={26}/><span className="lab-range-label">MID-TEMPERATURE</span><h3>Matched to your<br/>operating window.</h3><p>For lab processes that need cooling within a defined mid-temperature range. Share your setpoint and heat load to establish the configuration.</p><a className="lab-text-link" href={inquiry}>Define your temperature range <ArrowRight size={16}/></a></article>
          <article className="lab-low-temp"><Snowflake size={26}/><span className="lab-range-label">LOW-TEMPERATURE</span><p className="lab-temperature-number"><span>DOWN TO</span>−40<small>°C</small></p><p>For applications requiring lower process temperatures. Cooling capacity at your setpoint and fluid requirements are confirmed during system selection.</p><a className="lab-text-link" href={inquiry}>Discuss low-temperature duty <ArrowRight size={16}/></a></article>
        </div>
      ...
      <section className="lab-wrap lab-section" id="planning" aria-labelledby="lab-planning-title">
        <div className="lab-heading-row"><div><p className="lab-kicker">03 / BUILT AROUND THE APPLICATION</p><h2 id="lab-planning-title">Let’s define<br/>your process fit.</h2></div><p>You don’t need to arrive with a finished specification. These four details give us a useful starting point.</p></div>
      ...
      <section className="lab-family"><div className="lab-wrap lab-family-grid"><div><p className="lab-kicker">THE RIGHT PRODUCT FAMILY</p><h2>A lab chiller line.<br/>A separate purpose.</h2></div><div><p><strong>PERMA Lab Process™</strong> is the split-system lab process cooling line, with a remote condenser and mid-temperature to low-temperature configurations.</p><p><strong>BLAST™</strong> is Perma Cool’s ethanol extraction chiller series.</p><a className="lab-text-link" href="/ethanol-chilling-systems">Looking for ethanol extraction chilling? <ArrowUpRight size={18}/></a></div></div></section>
      ...
      <section className="lab-wrap lab-section lab-faq" aria-labelledby="lab-faq-title"><div><p className="lab-kicker">A FEW USEFUL DETAILS</p><h2 id="lab-faq-title">Before we begin.</h2></div><div>
        <details><summary>What does “split-system process” mean?</summary><p>The lab-side process cooling equipment and the remote condenser are installed in separate locations, connected by refrigeration lines. The arrangement is designed around the process and facility.</p></details>
        ...
        <details><summary>Does every configuration operate at −40 °C?</summary><p>No. This line covers mid-temperature and low-temperature duties. The selected configuration must be matched to your setpoint, process load, fluid, and operating conditions.</p></details>
      </div></section>

      <section className="lab-final"><div className="lab-wrap"><p className="lab-kicker">PERMA LAB PROCESS™</p><h2>Bring us the process.<br/><em>Let’s plan the cooling.</em></h2><p>Tell us what you need to cool, the temperature you need, and where the system will be installed.</p><div className="lab-actions"><a className="lab-button lab-button-primary" href={inquiry}>Contact us about Lab Process <ArrowUpRight size={18}/></a><a className="lab-text-link" href="tel:+17472081001">747.208.1001</a></div><span className="lab-final-mark" aria-hidden="true"><Snowflake/></span></div></section>
    </main>
  );
}
